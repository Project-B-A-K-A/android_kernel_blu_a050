...
#else
#define    FTS_PACKET_LENGTH               128  // 2 data length
...
u8 fts_cmd_write(u8 btcmd,u8 btPara1,u8 btPara2,u8 btPara3,u8 num)
{
 ...
 ssize_t fts_dma_write_m_byte(unsigned char*returnData_va, u32 returnData_pa, int  len)
{
    int     rc=0;
    if (len > 0)
    {
 ...
 ssize_t fts_dma_read_m_byte(unsigned char cmd, unsigned char*returnData_va, u32 returnData_pa,unsigned char len)
{
    int rc=0;
...
/*send the opration head */
	CTP_DBG("read out checksum\n");
	buf[0] = 0x64;
       i2c_master_send(g_pts->client, buf, 1);
	msleep(300);
	...
	buf[4] = (u8)(temp >> 8);
	buf[5] = (u8)(temp);
       i2c_master_send(g_pts->client, buf, 6);
	msleep(dw_lenth/256);
...
static ssize_t fts_bin_write_block(uint8_t* pbt_buf, int dw_lenth)
{
	...
	if ((dw_lenth) % FTS_PACKET_LENGTH > 0)
	{
		...
		ret = fts_dma_write_m_byte(tpDMABuf_va, tpDMABuf_pa,temp+6);    
	       if (ret < 0) 
	       {
	            CTP_DBG("[Error]TP write data error!! temp=%d\n", temp);
	            return 0;
	...
	CTP_DBG("read out checksum\n");
	packet_buf[0] = 0x64;
       i2c_master_send(g_pts->client, packet_buf, 1);
	msleep(300);
	...
	packet_buf[4] = (u8)(temp >> 8);
	packet_buf[5] = (u8)(temp);
       i2c_master_send(g_pts->client, packet_buf, 6);
	msleep(dw_lenth/256);
