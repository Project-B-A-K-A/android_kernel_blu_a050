...
#endif
#define IOCTL_GET_POWER_UP							_IOR(ELAN_IOCTLID, 22, int)
#define IOCTL_GET_POWER_DOWN							_IOR(ELAN_IOCTLID, 23, int)
...
{
    printk("mtk-tpd:[elan] elan_power_up\n");
...
static void elan_ktf2k_ts_report_data(struct i2c_client *client, uint8_t *buf)
{
   ...
   struct  hwm_sensor_data sensor_data;
    int ret = 0;
#endif
    printk("elan_ktf2k_ts_report_data buf[0]=%x  buf[2] =%x\n",buf[0],buf[2]);
...
#ifdef CONFIG_PROJECT_V2500_BLU
...
//    printk("CTP_Face_Mode_Switch  onoff_state %d, PROXIMITY %d\n",onoff_state, PROXIMITY);
    printk("CTP_Face_Mode_Switch  onoff_state %d, PROXIMITY %d i2c_client =%x\n",onoff_state, PROXIMITY,i2c_client->addr);

    rc = i2c_master_send( i2c_client, &bWriteData[0], 4);
    return rc ;
}
static  int Get_Ctp_Face_Mode(void)
{
