...
#define TPD_KEYS                {KEY_BACK, KEY_HOMEPAGE, KEY_MENU,KEY_SURPLUS}
...
static void elan_ktf2k_ts_report_data(struct i2c_client *client, uint8_t *buf)
{
    ...
    if (buf[0] == 0x55)
    {
        ...
        //    CTP_DBG("mtk-tpd:[elan] 55 55 55 55--PS ON---\n", __func__);
        }
    }
    else if(buf[0] == 0xFA)
    {
        ...
        {
            PROXIMITY_STATE = 0;
            CTP_DBG("tpd_touchinfo PROXIMITY_STATE %d\n",PROXIMITY_STATE );
        }
        ...
        if (PROXIMITY == 1)
        {
            ...
            if (ret)
            {
                CTP_DBG("Call hwmsen_get_interrupt_data fail = %d\n", ret);
            }
        }
    }
#endif
    ...
    switch (buf[0])
    {
        ...
        case TEN_FINGERS_PKT:
                ...
                for (i = 0; i < finger_num; i++)
                {
                    if ((fbits & 0x01))
                    {
                        ...
                        if (!((x<=0) || (y<=0) || (x>=X_RESOLUTION) || (y>=Y_RESOLUTION)))
                        {
                            if ( y < limitY )
                            {
                                CTP_DBG("mtk-tpd elan_ktf2k_ts_report_data x=%d y=%d id=%d \n",x,y,i);
            ...
            else
            {
                input_mt_sync(idev);
                input_sync(idev);
                CTP_DBG("mtk-tpd elan_ktf2k_ts_report_data up\n");
            }

            break;
        default:
            CTP_DBG("[elan] %s: unknown packet type: %0x\n", __func__, buf[0]);
            break;
    } // end switch
    return;
}
...

{
    ...
    CTP_DBG("CTP_Face_Mode_Switch  onoff_state %d, PROXIMITY %d\n",onoff_state, PROXIMITY);

    return i2c_master_send( i2c_client, &bWriteData[0], 4);
}
static  int Get_Ctp_Face_Mode(void)
{
    CTP_DBG("Get_Ctp_Face_Mode PROXIMITY_STATE %d\n",PROXIMITY_STATE);
                      ...
                      void *buff_out, s32 size_out, s32 *actualout)
{
    ...
    {
        case SENSOR_DELAY:
            CTP_DBG("hdx ektf2k_ps_operate SENSOR_DELAY\n");
        ...
        case SENSOR_ENABLE:
            CTP_DBG("hdx ektf2k_ps_operate SENSOR_ENABLE");
            if ((buff_in == NULL) || (size_in < sizeof(int)))
            {
        ...
        case SENSOR_GET_DATA:
            CTP_DBG("hdx ektf2k_ps_operate SENSOR_GET_DATA\n");
...
static int tpd_suspend(struct i2c_client *client, pm_message_t message)
{
    ...
    CTP_DBG("mtk-tpd:[elan] TP enter into sleep mode\n");
    if(work_lock == 1) //updating or doing something else
    {
    ...
    {
        PROXIMITY_SLEEP = 1;
        CTP_DBG("mtk-tpd:[elan] TP enter into sleep mode  ps return!\n");
        return ;
    }
...
static int tpd_resume(struct i2c_client *client)
{
    ...
    if( PROXIMITY== 1)//PROXIMITY_SLEEP
    {
        ...
        {
            PROXIMITY_SLEEP = 0;
            CTP_DBG("mtk-tpd:[elan]tpd_resume TPD wake up ps return!\n");
            return ;
        }
    }
#endif //TP_PROXIMITY_SENSOR_NEW
