...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev,
			...
			fwu->config_data[3]);
             ...
             CTP_DBG("%s: line=%d,ftm_force_update=%d \n",__func__,__LINE__,ftm_force_update);
...
int fwu_IMG_ID(void)
{
	...
	if (header.config_size) {
        ...
        printk("==== fwu_IMG_ID=%d====", extract_uint_be(fwu->config_data));
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		CTP_DBG("%s: Firmware image size = %d\n",
				__func__, fw_entry->size);
	...
	flash_area = fwu_go_nogo(&header);
             CTP_DBG("flash_area=%d",flash_area);
	switch (flash_area) {
	case UI_FIRMWARE:
		retval = fwu_do_reflash();
		break;
...
int synaptics_get_fw_version(void)
{
        ...
        retl = retl - 0x30;
        ret = reth*10 + retl;       
        printk("%s:gconfig_id=%x,ret=%d",__func__,gconfig_id,ret);
        //return gconfig_id;
        return ret;
}
