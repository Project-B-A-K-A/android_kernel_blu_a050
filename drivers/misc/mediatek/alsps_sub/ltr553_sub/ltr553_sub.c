...
static struct ltr553_sub_priv *ltr553_sub_obj = NULL;
 ...
 */
static int ltr553_sub_get_ps_value(struct ltr553_sub_priv *obj, int ps);
...
static int ltr553_sub_enable_ps(struct i2c_client *client, bool enable)
{
    struct ltr553_sub_priv *obj = i2c_get_clientdata(client);
    int err=0;
    int trc = atomic_read(&obj->trace);
...
static DRIVER_ATTR(gsensorState,   S_IWUSR | S_IRUGO, ltr553_sub_show_gsensorState, ltr553_sub_store_gsensorState);
static DRIVER_ATTR(audioChannel,     S_IWUSR | S_IRUGO, show_audioChannel, store_audioChannel); //guomingyi 20140812 add for cat ps chip info
...
/*----------------------------------------------------------------------------*/
static int  ltr553_sub_get_ps_value_gesture(struct ltr553_sub_priv *obj, int ps)
{
    int val= -1;
    int invalid = 0;
