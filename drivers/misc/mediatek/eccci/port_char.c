...
char modem_img_name[30];
printk("========LGC=====CCCI_IOC_GET_MD_IMG_EXIST==========Market_Area=%s===================\n",Market_Area);
snprintf(modem_img_name, IMG_NAME_LEN, "E%s.img",Market_Area);
	//	scaned_num = scan_image_list(md->index, "modem_%d_%s_n.img", tmp_md_img_list, MAX_IMG_NUM);
	scaned_num = scan_image_list(md->index, "modem_%d_%s_%s.img", tmp_md_img_list,Market_Area, MAX_IMG_NUM);
#else
