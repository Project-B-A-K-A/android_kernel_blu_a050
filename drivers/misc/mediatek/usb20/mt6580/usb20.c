...
static void uart_usb_switch_dump_register(void)
{
	...
	DBG(0, "[MUSB]addr: 0x10005000 (GPIO), value: %x\n\n", DRV_Reg8(ap_gpio0_base + 0x118));
}
...
static ssize_t mt_usb_show_uart_path(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	var = DRV_Reg8(ap_gpio0_base + 0x118);
...
#ifdef CONFIG_MTK_UART_USB_SWITCH
	struct device_node *ap_gpio0_node = NULL;
#endif
#endif
	int				ret = -ENOMEM;
...
#ifdef CONFIG_MTK_UART_USB_SWITCH
	ap_gpio0_node = of_find_compatible_node(NULL, NULL, AP_UART0_COMPATIBLE_NAME);

	if (ap_gpio0_node == NULL) {
		dev_err(&pdev->dev, "USB get ap_uart0_node failed\n");
		if (ap_gpio0_base)
			iounmap(ap_gpio0_base);
		ap_gpio0_base = 0;
	} else {
		ap_gpio0_base = of_iomap(ap_gpio0_node, 0);
	}
#endif
